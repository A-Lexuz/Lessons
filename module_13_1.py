...
async def start_strongman(name,power):
    cprint(f'Силач {name} начал соревнования.', 'red', force_color=True)
    for i in range(5):
        await asyncio.sleep(6-power)
        cprint(f'Силач {name} поднял {i+1} шар.', 'white', 'on_black', force_color=True)
    cprint(f'Силач {name} закончил соревнования.', 'green', force_color=True)
